...
export async function POST(
  ...
  try {
    ...
    const selectedYacht =
      await loadSelectedProposalYacht(
        admin,
        workspace.companyId,
        proposalId,
...
async function loadProposal(
  ...
  const result =
    await admin
      .from("inquiries")
      .select(
        "id, reference, company_id, client_name, email, phone, destination, start_date, end_date, guests, currency, proposal_created_at"
      )
      .eq(
        "company_id",
        companyId
...
async function loadSelectedProposalYacht(
  ...
  const result =
    await admin
      ...
      .select(
        "id, proposal_id, fleet_id, yacht_name, weekly_rate, estimated_total, currency"
      )
      .eq(
        "company_id",
        companyId
