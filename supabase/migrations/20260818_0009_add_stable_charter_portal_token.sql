...
comment on column public.guest_portals.token_encrypted is
  'Encrypted raw Charter Portal token. Public lookup still uses token_hash; the encrypted value exists only so Bahari OS can resend the same secure client link without rotating it.';
